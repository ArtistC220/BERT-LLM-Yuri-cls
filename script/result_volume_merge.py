...
def merge_csv_files(history_dir, new_csv_dir, new_csv_name):
    ...
    # 去重（以 aid+vid 为唯一标识，保留新文件的记录）
    merged_df = merged_df.drop_duplicates(subset=["aid","vid"], keep="last")
...
if __name__ == "__main__":
    # ====== 你需要修改的部分 ======
    history_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "csv","history_rank_volume_large")  # 主排行榜目录
    new_csv_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "csv","result")   # 新排行榜目录
    new_csv_name = "volume_result_sorted.csv"  # 你指定的新排行榜文件名
    # =============================
